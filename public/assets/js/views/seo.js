...
const METRIC_LABELS = {
    ...
    ai: 'Visits from AI assistants'
};

/** The sentence an empty movers table shows, by ranking. */
const EMPTY_REASONS = {
    gain: 'Nothing grew against the comparison period.',
    loss: 'Nothing dropped against the comparison period.',
    now: 'No visits in this period.',
    new: 'Nothing had visits in this period that had none in the comparison period.',
    gone: 'Nothing that had visits in the comparison period is missing from this one.'
};
...
function share(value, total) {
    return total > 0 ? dec((value / total) * 100, 1) + '%' : 'no visits';
}
...
function dual(kind, a, b) {
    return el('span', { class: 'seo-dual' }, [
        ...
        el('span', { class: 'seo-dual-before', text: 'was ' + fmt(kind, b) }),
...
function ignoredNote(id, list) {
    ...
    if (Array.isArray(list) && list.length) {
        node.textContent = 'Not applied to crawler requests, because a request does not carry it: ' + list.join(', ') + '.';
        node.hidden = false;
        return;
    }
    node.textContent = '';
...
async function loadBounds() {
    ...
    if (note) {
        note.textContent = 'History starts ' + when(data.earliest);
        note.hidden = false;
    }
}
...
function loadScorecard() {
    return loadCard('seo-scorecard', 'Comparing the two periods', async () => {
...
function renderTiles(data) {
    ...
    setPop('seo-scorecard', 'Visits that loaded at least one page, counted in the period they arrived in, under every '
        + 'filter in force. Median engaged time covers finished visits the beacon measured: '
        + num(data.engaged_n.a) + ' in this period, ' + num(data.engaged_n.b) + ' in the comparison period.');
    ...
    mount.replaceChildren(...data.metrics.map((m) => el('div', { class: 'seo-tile', title: m.hint }, [
        ...
        el('span', { class: 'seo-tile-before', text: 'Compared with ' + fmt(m.kind, m.b) }),
...
function drawTimeline() {
    ...
    if (note) {
        note.textContent = METRIC_LABELS[metric] + ' per ' + timelineData.step + '. Point by point, each period is drawn '
            + 'from its own start, so the lines line up by position in the period rather than by date.';
    }
}
...
function loadChannels() {
    return loadCard('seo-channels', 'Comparing channels', async () => {
        ...
        if (note) {
            ...
            note.textContent = series.series.length ? '' : 'No visits in this period to draw.';
        }
...
function renderChannels(data) {
    setPop('seo-channels', num(data.total_a) + ' visits in this period, ' + num(data.total_b) + ' in the comparison '
        + 'period. Direct means no referrer arrived, not that somebody typed your address.');
...
function renderMovers(id, spec, data) {
    ...
    setPop(id, data.chan_label + ' · ' + num(data.total_a) + ' visits in this period, ' + num(data.total_b)
        + ' in the comparison period. '
        + (data.capped
            ? 'Ranked from the ' + num(data.candidates) + ' busiest of ' + num(data.distinct) + ' values across both '
                + 'periods, so a value with little traffic in either can be missing. '
            : '')
    ...
    if (!data.rows.length) {
        ...
        showEmpty(id + '-empty', 'Nothing to list', [
            (EMPTY_REASONS[data.sort] || EMPTY_REASONS.now) + ' Channel: ' + data.chan_label + ', under every filter in force.'
        ]);
        return 'own';
    }
    hideEmpty(id + '-empty');
...
function loadQuality() {
    return loadCard('seo-quality', 'Measuring each channel', async () => {
    ...
    tbody(byId('seo-quality-table'), rows.map((row) => ({
        ...
        cells: [
            ...
            { node: dual('pct', row.a.one_page_share, row.b.one_page_share), class: 'num', sort: sortable(row.a.one_page_share) },
            {
                ...
                class: 'num',
                title: num(row.a.engaged_n) + ' visits measured in this period, ' + num(row.b.engaged_n) + ' in the comparison period',
...
function loadCrawlers() {
    return loadCard('seo-crawlers', 'Counting crawler requests', async () => {
...
function verifiedNode(half) {
    if (!half.checked) {
        return el('span', { class: 'muted', text: 'not checked' });
    }
    return el('span', { class: 'mono', text: num(half.verified) + ' of ' + num(half.checked) });
}

/** Fill the crawler table. */
function renderCrawlers(data) {
    ignoredNote('seo-crawlers', data.ignored);
    setPop('seo-crawlers', data.category_label + ' · ' + num(data.total_a) + ' requests in this period, '
        + num(data.total_b) + ' in the comparison period.'
    ...
    if (!data.rows.length) {
        ...
        showEmpty('seo-crawlers-empty', 'No crawler requests', [
            'No declared crawler of this kind fetched anything in either period under the filters in force.'
        ]);
        return;
    }
    ...
    tbody(byId('seo-crawlers-table'), data.rows.map((row) => ({
        cells: [
            ...
            {
                node: row.category ? dimValue('ua_bot_cat_s', row.category) : el('span', { class: 'muted', text: 'not recorded' }),
                sort: row.category
            },
            ...
            { node: dual('count', row.a.e5, row.b.e5), class: 'num', sort: row.a.e5 },
            {
                ...
                class: 'num',
                title: 'In the comparison period: ' + (row.b.checked ? num(row.b.verified) + ' of ' + num(row.b.checked) : 'not checked'),
...
function loadGap() {
    return loadCard('seo-crawlgap', 'Matching crawled pages against visits', async () => {
...
function renderGap(data) {
    ...
    setPop('seo-crawlgap', data.engine_label + ' · the ' + num(data.examined) + ' pages crawled most with a 2xx answer '
    ...
    if (!data.rows.length) {
        ...
        showEmpty('seo-crawlgap-empty', 'Nothing to list', [
            data.sort === 'unvisited' && data.examined
                ? 'Every page crawled most in this period also received visits from the matching channel.'
                : 'No crawler of this kind fetched a page with a 2xx answer in this period under the filters in force.'
        ]);
        return;
    }
    ...
    tbody(byId('seo-crawlgap-table'), data.rows.map((row) => ({
        cells: [
            ...
            row.visits === null
                ? { text: 'not checked', class: 'num muted', sort: -1 }
