...
final class Solr
{
    ...
    /**
     * Search the VALUES of one field for a substring, and return the matching terms with counts.
     *
     * WHY THIS IS CLASSIC FACETING AND NOT A JSON FACET, which is the surprise in this file.
     * Everything else the panel aggregates goes through jsonFacet(). This one cannot, because
     * **the JSON Facet API's terms facet has no substring filter**. `contains` and
     * `containsIgnoreCase` are parameters of the CLASSIC facet component; passed inside a
     * json.facet block they are neither honoured nor rejected — they are silently dropped.
     * Measured against a live Solr node while this was written:
     *
     ...
     *       → numBuckets 8, all of them beginning "New"        (prefix IS supported)
     *
     ...
     *
     * That difference is the whole reason this method exists. A search box built on the JSON
     * facet spelling would have looked like it worked, returned the most common values of the
     * whole dimension whatever was typed, and been believed — which is worse than no search box,
     * and is exactly the class of silently-wrong number this project refuses to ship.
     *
     * Tag exclusion still applies and still matters: the dimension being searched must ignore its
     * OWN filter, or searching inside a filtered dimension returns only what is already selected.
     * The classic spelling of that is a local param on the facet field itself,
     * `facet.field={!ex=f_host_s}host_s`, and it is the one place a `{!…}` block is built here.
     * Both halves were verified together against a live node.
     *
     * THE SUBSTRING IS REQUEST TEXT — the first facet parameter in this codebase that is — so it
     * is treated exactly like a filter value: length-capped, control characters refused, and never
     * concatenated into anything. It travels as its own form-encoded parameter and Solr compares
     * it literally against each term, so there is no query syntax in it to escape; the cap is
     * there because a term scan is work, and the control-character check because a newline would
     * split the parameter.
     *
     * @param string              $field       Field to enumerate. Allowlist-checked.
     * @param string              $contains    Substring to match. 1-64 characters, no control chars.
     * @param array<string,mixed> $queryParams q/fq and the rest, exactly as query() takes them.
     * @param array<int,string>   $excludeTags Tags this facet ignores, usually its own.
     * @return array{buckets:array<int,array{val:string,count:int}>}
     */
    public function facetContains(
        string $core,
        string $field,
        string $contains,
        array $queryParams,
        ...
        $params['rows'] = 0;
        $params['facet'] = 'true';
        $params['facet.field'] = self::facetFieldWithExclusions($field, $excludeTags);
        $params['facet.limit'] = Security::clampInt($limit, 1, 2000, 200);
        $params['facet.mincount'] = 1;
        $params['facet.sort'] = 'count';
        $params['facet.contains'] = self::assertSafeContains($contains);
        $params['facet.contains.ignoreCase'] = $ignoreCase ? 'true' : 'false';

        $resp = $this->send(
            $core,
            '/select',
            [],
            self::encodeParams($this->sanitiseQueryParams($params)),
            'POST',
            'application/x-www-form-urlencoded'
        );

        $flat = $resp['facet_counts']['facet_fields'][$field] ?? null;
        if (!is_array($flat)) {
            return ['buckets' => []];
        }
        ...
        for ($i = 0; $i + 1 < count($flat); $i += 2) {
            $buckets[] = ['val' => (string) $flat[$i], 'count' => (int) $flat[$i + 1]];
        }
        return ['buckets' => $buckets];
    }

    /**
     * Build a classic `facet.field` value, optionally carrying tag exclusions.
     *
     * The only `{!…}` block this class constructs outside a filter. Every tag is generated by
     * the caller from an allowlisted field name and is re-checked here against the same pattern
     * Solr's local-param grammar accepts, so nothing that reaches this string came from a request.
     *
     * @param array<int,string> $excludeTags
     */
    private static function facetFieldWithExclusions(string $field, array $excludeTags): string
    {
        $tags = [];
        ...
        return '{!ex=' . implode(',', $tags) . '}' . $field;
    }

    /**
     * Remove a leading tag-exclusion block from a `facet.field` value, for validation.
     *
     * The classic spelling of "this facet ignores its own filter" is a local param ON THE FIELD
     * NAME — `{!ex=f_host_s}host_s` — so the field-name check has to see past it or it refuses
     * the one construct that makes a multi-select facet work. ONLY `{!ex=…}` with a tag list is
     * skipped, and only at the start; anything else is left in place and fails the allowlist,
     * which is what keeps `{!frange}`, `{!join}` and `{!xmlparser}` out of a parameter that
     * would otherwise accept them.
     */
    private static function stripFacetExclusion(string $value): string
    {
        return (string) preg_replace('/^\{!ex=[A-Za-z0-9_,]{1,128}\}/', '', trim($value));
    }

    /**
     * Validate a facet substring.
     *
     * Solr compares this literally against each indexed term, so it carries no query syntax and
     * needs no escaping — which is precisely why it needs stating, because "no escaping required"
     * reads like "unchecked" in a diff.
     *
     * That claim is MEASURED, not assumed. Against a live node, with the rest of the query held
     * constant at 485 documents:
     *
     *     facet.contains={!frange l=0 u=100}  → numFound 485, 0 matching terms
     *     facet.contains=_query_:"*:*"        → numFound 485, 0 matching terms
     *     facet.contains=") OR (""="          → numFound 485, 0 matching terms
     *     facet.contains=API                  → numFound 485, 6 matching terms
     *
     * None of them changed the result set or was evaluated as anything; each was looked for as a
     * run of characters and not found. So `{!`, `_query_` and `_val_` are ACCEPTED here, unlike in
     * a filter where they are refused outright — and they have to be, because a path, a
     * User-Agent or an organisation name can legitimately contain any of them and an operator
     * searching for one must be able to type it.
     *
     * What IS required:
     *
     *   - a control character would split or truncate the form-encoded parameter, so it is
     *     refused rather than stripped: stripping joins the two halves into a third string the
     *     reader never typed and then shows them results for it;
     *   - a length cap, because matching a substring means walking the field's term dictionary
     *     and the cost is the caller's to bound;
     *   - non-empty, so an empty box cannot become a full enumeration of the term space.
     */
    public static function assertSafeContains(string $contains): string
    {
        $contains = trim($contains);
        if ($contains === '') {
            throw new \InvalidArgumentException('Solr: empty facet substring.');
        }
    ...
    private function sanitiseQueryParams(array $params): array
    {
        ...
        foreach (['facet.field', 'facet.pivot', 'stats.field', 'group.field', 'df'] as $fieldParam) {
            ...
            foreach ($names as $name) {
                foreach (explode(',', self::stripFacetExclusion((string) $name)) as $one) {
                    $one = trim($one);
        ...
        if (isset($out['facet.contains'])) {
            $out['facet.contains'] = self::assertSafeContains((string) $out['facet.contains']);
        }
     ...
     *
     * `numBuckets` is a plain boolean and is cast rather than validated, because there is
     * nothing in it a caller could get wrong. It is permitted because a value browser has to
     * be able to say honestly that it is showing the top N of M distinct values, and without
     * it the only alternatives are an unbounded facet or a truncation the reader cannot see.
     * It is not free — Solr counts the whole term space rather than the page being returned —
     * so callers ask for it only where the number is going on screen.
     *
