...
STUB);
    ...
    // ---------------------------------------------------------------------------------
    // It removes what it wrote, and only what it wrote
    // ---------------------------------------------------------------------------------

    'a web server configuration the installer did not write is never deleted'
        => static function (): void {
            $root = lh_un_paths()['root'];
            $sh = $root . '/install/install.sh';
            $src = (string) file_get_contents($sh);

            lh_contains($src, 'installer_wrote()', 'the ownership check exists');
            lh_true(
                strpos($src, 'if ! installer_wrote "$proof"') < strpos($src, 'run rm -f "$file"'),
                'and a vhost is proven ours BEFORE anything deletes it'
            );

            // Exercise the real function against real files. A banner is the only proof
            // accepted, because a filename is not evidence of authorship — deleting
            // sites-available/loghound.conf because of its NAME is how an uninstaller
            // destroys a vhost the operator wrote themselves.
            $dir = lh_tmpdir('lh-own');
            $fixtures = [
                'generated-apache.conf' => "# Loghound panel — GENERATED by install/install.sh. Do not hand-edit;\n<VirtualHost *:443>\n</VirtualHost>\n",
                'generated-pool.conf'   => "; Loghound — dedicated PHP-FPM pool. Generated by install/install.sh.\n[loghound]\n",
                'handwritten.conf'      => "# I wrote this myself, and it points at the panel.\n<VirtualHost *:443>\n</VirtualHost>\n",
                'neighbour.conf'        => "<VirtualHost *:443>\n  ServerName shop.example.com\n</VirtualHost>\n",
                'www.conf'              => "[www]\nuser = www-data\n",
                'late-banner.conf'      => str_repeat("# padding\n", 20) . "# GENERATED by install/install.sh\n",
            ];
            ...
            $fn = $dir . '/fn.sh';
            preg_match('/^installer_wrote\(\) \{.*?^\}$/ms', $src, $m);
            lh_true($m !== [], 'the function could be extracted');
            file_put_contents($fn, $m[0]);

            $verdict = static function (string $file) use ($fn): bool {
                $cmd = '. ' . escapeshellarg($fn) . ' && installer_wrote ' . escapeshellarg($file);
                exec('bash -c ' . escapeshellarg($cmd), $out, $code);
                return $code === 0;
            };

            lh_true($verdict($dir . '/generated-apache.conf'), 'a generated vhost is ours to remove');
            lh_true($verdict($dir . '/generated-pool.conf'), 'and so is a generated pool');

            lh_false(
                $verdict($dir . '/handwritten.conf'),
                'a vhost the operator wrote for the panel is NOT ours, however it is named'
            );
            lh_false($verdict($dir . '/neighbour.conf'), 'and neither is somebody else\'s site');
            lh_false($verdict($dir . '/www.conf'), 'nor the distribution\'s own FPM pool');
            lh_false(
                $verdict($dir . '/late-banner.conf'),
                'the banner has to be at the top: a file mentioning install.sh halfway down is '
                . 'not a file install.sh wrote'
            );
            lh_false($verdict($dir . '/does-not-exist.conf'), 'and an unreadable file fails closed');

            lh_rmtree($dir);
        },

    'the uninstaller never appends to a file it did not create' => static function (): void {
        $src = (string) file_get_contents(lh_un_paths()['root'] . '/install/install.sh');

        // An append into a shared file is the one edit that cannot be reversed by deleting a
        // file, so the rule is that it never happens. If it ever has to, it must be a marked
        // block removed by its markers — and this is the test that makes somebody read that
        // sentence before adding the first one.
        $appends = [];
        foreach (explode("\n", $src) as $n => $line) {
            if (preg_match('/>>\s*"?\$/', $line) !== 1) {
                continue;
            }
            if (str_contains($line, 'INSTALL_LOG') || str_contains($line, 'STATE_SNAPSHOT')) {
                continue;
            }
            $appends[] = ($n + 1) . ': ' . trim($line);
        }

        lh_same(
            [],
            $appends,
            'install.sh appends to something that is not its own log or state file. If that is '
            . 'deliberate it has to be a marked block the uninstaller can remove by its markers, '
            . 'and the uninstaller has to learn to do that'
        );
    },

    'the report names the edit in the operator\'s own file that only they can undo'
        => static function (): void {
            $src = (string) file_get_contents(lh_un_paths()['root'] . '/install/install.sh');

            lh_contains($src, 'LogFormat', 'the report mentions the LogFormat line');
            lh_contains(
                $src,
                'never truncated, never rotated',
                'and states what was done to the source logs, which is nothing'
            );
            lh_contains($src, 'What was NOT removed', 'under a heading that says what the list is');
        },
